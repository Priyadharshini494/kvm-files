...
import {Recorder2} from "./recorder.js";
import {Recorder4} from "./recorder.js";
...
export function Session() {
	...
	var __setExtras = function(state) {
		...
		__streamer2.setJanusEnabled(
			(state.janus && (state.janus.enabled || state.janus.started))
			|| (state.janus_static && (state.janus_static.enabled || state.janus_static.started))
		);
		__streamer3.setJanusEnabled(
			(state.janus && (state.janus.enabled || state.janus.started))
			|| (state.janus_static && (state.janus_static.enabled || state.janus_static.started))
		);
		__streamer4.setJanusEnabled(
			(state.janus && (state.janus.enabled || state.janus.started))
			|| (state.janus_static && (state.janus_static.enabled || state.janus_static.started))
		);
	};
		...
		let http = tools.makeRequest("GET", "/api/auth/check", function() {
			if (http.readyState === 4) {
				if (http.status === 200) {
					...
					tools.info("Location host", location.host);
	...
	var __wsOpenHandler = function(event) {
		...
		$("link-led2").className = "led-green";
		$("link-led2").title = "Connected";
		$("link-led3").className = "led-green";
	...
	var __wsMessageHandler = function(event) {
		 tools.info("Session: received socket data:", event.data);
		let data = JSON.parse(event.data);
		console.log("Cookies:",tools.cookies.get("stream_client"));
