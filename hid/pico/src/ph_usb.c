...
const u16 *tud_descriptor_string_cb(u8 index, u16 lang_id) {
	...
	if (index == 0) {
		...
		char str[32];
		switch (index) {
			case 1: strcpy(str, "Rutomatrix"); break; // Manufacturer
			case 2: strcpy(str, (ph_g_is_bridge ? "Rutomatrix HID Bridge" : "Rutomatrix HID")); break; // Product
			case 3: pico_get_unique_board_id_string(str, 32); break; // Serial
			case 4: {
					if (ph_g_is_bridge) {
						strcpy(str, "Rutomatrix HID Bridge CDC");
					} else {
						return NULL;
					}
				}; break;
