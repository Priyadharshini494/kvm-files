...
class _StreamerApiPart2(_BaseApiPart):
    async def get_state(self) -> dict:
        session = self._ensure_http_session()
        async with session.get(self._make_url("streamer2")) as response:
            htclient.raise_not_200(response)
            return (await response.json())["result"]
...
class RutomatrixClientSession:
    def __init__(
        ...
        self.streamer2 = _StreamerApiPart2(*args)
        self.hid = _HidApiPart(*args)
        self.atx = _AtxApiPart(*args)

    @contextlib.asynccontextmanager
    async def ws(self) -> AsyncGenerator[RutomatrixClientWs, None]:
        session = self.__ensure_http_session()
        async with session.ws_connect(self.__make_url("ws")) as ws:
            yield RutomatrixClientWs(ws)
    ...
    async def __aenter__(self) -> "RutomatrixClientSession":
        return self
...
class RutomatrixClient:
    ...
    def make_session(self, user: str, passwd: str) -> RutomatrixClientSession:
        return RutomatrixClientSession(
